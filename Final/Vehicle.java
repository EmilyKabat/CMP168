...
public abstract class Vehicle {
    ...
    public int getNumberOfAvailableSeats() {
        ...
        return numSeats;
//        do i need to subtract num people?
    }
    public int getNumberOfAvailableSeatsInRow(int row) {
        if (row < 0 || row > numberOfRows) {
            return -1;
        }
        return numSeatsPerRow[row];
//        do i need to subtract num people in row?
    }
    ...
    public Person[] getPeopleInRow(int row) {
        Person[] peopleInRow = new Person[maxSeatsPerRow];
        for (int i = 0; i < maxSeatsPerRow; i++) {
            if (personsOnBoard[row][i] != null) {
                peopleInRow[i] = personsOnBoard[row][i];
            }
        }
        return peopleInRow;
//        deal w invalid rows/rows w no one in them
    }
